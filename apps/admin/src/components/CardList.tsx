  ...
  if (title === "Popular Products") {
    try {
      const res = await fetch(
        `${process.env.SERVER_PRODUCT_SERVICE_URL}/products?limit=5&popular=true`,
        { next: { revalidate: 0 } },
      );
      if (res.ok) {
    ...
    try {
      const res = await fetch(
        `${process.env.SERVER_ORDER_SERVICE_URL}/orders?limit=5`,
        {
          headers: {
        ...
        {title === "Popular Products"
          ? products?.map((item) => (
              ...
              >
                ...
                <CardFooter className="p-0">${item.price / 100}K</CardFooter>
              </Card>
            ))
          : orders?.map((item) => (
              <Card
