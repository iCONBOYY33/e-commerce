...
//   ];
// };
const getOrdersData = async (): Promise<OrderType[]> => {
  try {
    ...
    if (!ordersFetch.ok) {
      console.error(
        `API error: ${ordersFetch.status} ${ordersFetch.statusText}`
      );
      return [];
    }

    const data = await ordersFetch.json();
    console.log("API response in admin:", data);
